...
function ciniki_artgallery_fileDelete(&$ciniki) {
    ...
    $rc = ciniki_artgallery_checkAccess($ciniki, $args['business_id'], 'ciniki.artgallery.fileDelete'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
	...
	//
	$strsql = "DELETE FROM ciniki_artgallery_files "
		. "WHERE business_id = '" . ciniki_core_dbQuote($ciniki, $args['business_id']) . "' "
		. "AND id = '" . ciniki_core_dbQuote($ciniki, $args['file_id']) . "' "
		. "";
	...
	$rc = ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.artgallery', 'ciniki_artgallery_history', 
		$args['business_id'], 3, 'ciniki_artgallery_files', $args['file_id'], '*', '');
	...
	$ciniki['syncqueue'][] = array('push'=>'ciniki.artgallery.file', 
