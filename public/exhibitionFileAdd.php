...
// exhibition_id:            The ID of the exhibition the file is attached to.
...
// <rsp stat='ok' id='34' />
//
function ciniki_artgallery_exhibitionFileAdd(&$ciniki) {
    //  
    ...
    $rc = ciniki_artgallery_checkAccess($ciniki, $args['tnid'], 'ciniki.artgallery.exhibitionFileAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artgallery.51', 'msg'=>'You already have a file with this name, please choose another name'));
    }
    ...
    if( isset($_FILES['uploadfile']['error']) && $_FILES['uploadfile']['error'] == UPLOAD_ERR_INI_SIZE ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artgallery.52', 'msg'=>'Upload failed, file too large.'));
    }
    ...
    if( !isset($_FILES) || !isset($_FILES['uploadfile']) || $_FILES['uploadfile']['tmp_name'] == '' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artgallery.53', 'msg'=>'No file specified.'));
    }
    ...
    if( $args['extension'] != 'pdf' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artgallery.54', 'msg'=>'The file must be a PDF file.'));
    }
    ...
    //
    $storage_filename = $tenant_storage_dir . '/ciniki.artgallery/files/' . $args['uuid'][0] . '/' . $args['uuid'];
    if( !is_dir(dirname($storage_filename)) ) {
        if( !mkdir(dirname($storage_filename), 0700, true) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artgallery.55', 'msg'=>'Unable to add file'));
        }
    }

    if( !rename($_FILES['uploadfile']['tmp_name'], $storage_filename) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artgallery.56', 'msg'=>'Unable to add file'));
    }
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.artgallery.exhibition_file', $args, 0x07);
}
?>
