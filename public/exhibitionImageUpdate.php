...
function ciniki_artgallery_exhibitionImageUpdate(&$ciniki) {
	...
	//
	$strsql = "SELECT uuid, image_id FROM ciniki_artgallery_exhibition_images "
	...
	//
	$strsql = "UPDATE ciniki_artgallery_exhibition_images SET last_updated = UTC_TIMESTAMP()";
	...
	foreach($changelog_fields as $field) {
		if( isset($args[$field]) && $args[$field] != '' ) {
			...
			$rc = ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.artgallery', 
				...
				2, 'ciniki_artgallery_exhibition_images', $args['exhibition_image_id'], $field, $args[$field]);
		}
	}
	...
	if( !isset($rc['num_affected_rows']) || $rc['num_affected_rows'] != 1 ) {
		ciniki_core_dbTransactionRollback($ciniki, 'ciniki.artgallery');
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1127', 'msg'=>'Unable to update exhibition image'));	
	}
	...
	if( isset($args['image_id']) && $item['image_id'] != $args['image_id']) {
		...
		$rc = ciniki_images_refClear($ciniki, $args['business_id'], array(
			...
			'object_id'=>$args['exhibition_image_id']));
		if( $rc['stat'] == 'fail' ) {
			ciniki_core_dbTransactionRollback($ciniki, 'ciniki.artgallery');
			return $rc;
		}
